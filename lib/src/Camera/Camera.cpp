...
void Camera::setCentre(sf::Vector2f location) {
    ...
    // When the world is smaller than the viewport, centre on the world midpoint;
    // otherwise clamp so the camera does not move outside the world bounds.
    if (m_bounds.x <= getSize().x)
        location.x = getSize().x / 2;
    else
        location.x = std::clamp(location.x, halfSize.x, m_bounds.x - halfSize.x);

    if (m_bounds.y <= getSize().y)
        location.y = getSize().y / 2;
    else
...
void Camera::setSize(sf::Vector2f size) {
    m_view.setSize(size);
    setCentre(m_view.getCenter());  // re-clamp to new half-size
}
